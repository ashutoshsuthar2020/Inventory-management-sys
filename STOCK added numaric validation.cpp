...
void add_or_remove_stock(int x){
	...
	cin>>i_no;
    if((i_no<1)||(i_no>i))
    cout<<"Enter a suitbale item number.\n";
    else
    break;
    }

    while(true){
    if(x==1) cout<<"Enter the numbers of item to increase : ";
    else     cout<<"Enter the numbers of item to remove   : ";
    cin>>add_or_get;
    ...
    if(Category==1){
        ...
        *(data_produce[i_no-1].return_pointer_number_of_item()) +=add_or_get;
            ...
            *(data_produce[i_no-1].return_pointer_number_of_item()) -=add_or_get;
            ...
            cout<<"Not enough items in the Stock";
    }else if(Category==2){
        if(x==1)
